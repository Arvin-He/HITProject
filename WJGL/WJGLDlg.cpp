// WJGLDlg.cpp : implementation file
#include "stdafx.h"
#include "WJGL.h"
#include "WJGLDlg.h"
...
// CWJGLDlg dialog
CWJGLDlg::CWJGLDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CWJGLDlg::IDD, pParent)
	, m_editShutter(600)
	, m_editGain(10)
{
...
BEGIN_MESSAGE_MAP(CWJGLDlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_MESSAGE(WM_SNAP_CHANGE, OnSnapChange)	
	ON_MESSAGE(WM_SNAP_ERROR, OnSnapError)
	ON_MESSAGE(WM_SNAP_STOP, OnSnapStop)
	ON_COMMAND(IDM_OPEN, &CWJGLDlg::OnCamera_Open)	
	ON_COMMAND(IDM_STOP, &CWJGLDlg::OnCamera_Stop)
	ON_COMMAND(IDM_CLOSE, &CWJGLDlg::OnCamera_Close)
	ON_COMMAND(IDM_SAVE_PIC, &CWJGLDlg::OnCamera_SavePic)
	ON_COMMAND(IDM_SAVE_VIDEO, &CWJGLDlg::OnCamera_SaveVideo)
	ON_COMMAND(IDM_STOP_VIDEO, &CWJGLDlg::OnCamera_StopVideo)
	ON_COMMAND(IDM_SAVE_PARAM, &CWJGLDlg::OnSaveParam)
	ON_COMMAND(IDM_LOAD_PARAM, &CWJGLDlg::OnLoadParam)
	ON_WM_HSCROLL()
	ON_NOTIFY(TCN_SELCHANGE, IDC_TAB, &CWJGLDlg::OnTcnSelchangeTab)
	ON_NOTIFY(TCN_SELCHANGE, IDC_TAB2, &CWJGLDlg::OnTcnSelchangeTab2)
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_RBUTTONDOWN()
	ON_BN_CLICKED(IDC_SET_STAGE_X_HOME_BTN, &CWJGLDlg::OnClickedSetStageXHomeBtn)
	ON_BN_CLICKED(IDC_SET_STAGE_Y_HOME_BTN, &CWJGLDlg::OnClickedSetStageYHomeBtn)
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_STAGE_X_STOP_BTN, &CWJGLDlg::OnClickedStageXStopBtn)
...
BOOL CWJGLDlg::OnInitDialog()
{
	...
	m_imageAssistDlg.m_pWJGLDlg = this;
	// 初始化相机
	InitialDHCamera();
...
HCURSOR CWJGLDlg::OnQueryDragIcon()
{
...
//////////////////////////CCD函数///////////////////////////
//获取bayer格式
HV_BAYER_LAYOUT CWJGLDlg::HVGetBayerType(HHV hhv) 
{
	int nSize = 0; 
...
HVSTATUS CWJGLDlg::SetExposureTime(HHV hhv, int nWindWidth, long lTintUpper, long lTintLower, 
...
LRESULT CWJGLDlg::OnSnapError(WPARAM wParam, LPARAM lParam)
{	
	CErrorBox ErrDlg;
...
LRESULT CWJGLDlg::OnSnapStop(WPARAM wParam, LPARAM lParam)
{
	HVSTATUS status =STATUS_OK;
...
BOOL CWJGLDlg::DestroyDHCamera(HHV hDHCamera, BYTE *pRawBuffer, BYTE *pImageBuffer)
{
	HVSTATUS status = STATUS_OK;
	//	关闭数字摄像机，释放数字摄像机内部资源
...
HVSTATUS CWJGLDlg::GetLastStatus()
{
...
int CALLBACK CWJGLDlg::SnapThreadCallback(HV_SNAP_INFO *pInfo)
{
	CWJGLDlg *This = (CWJGLDlg *)(pInfo->pParam);
...
LRESULT CWJGLDlg::OnSnapChange(WPARAM wParam, LPARAM lParam)
{
	HVSTATUS status = STATUS_OK;	
...
void CWJGLDlg::ShowImage(Mat img, INT_PTR ID)
{
...
void CWJGLDlg::OnCamera_SavePic()
{
	//	以下保存BMP文件设置基本相同
...
void CWJGLDlg::OnCamera_SaveVideo()
{
...
UINT CWJGLDlg::StoreVideoThreadFunc(LPVOID lpParam)
{
	CWJGLDlg *pCWJGLDlg = (CWJGLDlg*)lpParam;
	pCWJGLDlg->StoreVideo();
	return 0;
}

void CWJGLDlg::StoreVideo()
{
	m_bIsCapture = TRUE;
	CString defaultDir = _T("D:\\image_avi_save");   //默认打开的文件路径
...
void CWJGLDlg::OnCamera_StopVideo()
{
	m_bIsCapture = FALSE;
}

void CWJGLDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
BOOL CWJGLDlg::SetGain(int ctrID)
{
	HVSTATUS status=STATUS_OK;
	long nGain;
	switch(ctrID)
...
void CWJGLDlg::OnSaveParam()
{
	WriteCCDConfig();
}

void CWJGLDlg::ReadCCDConfig()
{
	CString		fileName(_T(".\\config\\CCD.ini"));
	char gain[20];
...
void CWJGLDlg::OnLoadParam()
{
	ReadCCDConfig();
}

void CWJGLDlg::OnTcnSelchangeTab(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CWJGLDlg::OnTcnSelchangeTab2(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: Add your control notification handler code here
...
BOOL CWJGLDlg::ResetDMC3000Card()
{
...
// 画十字
void CWJGLDlg::DrawCross(Mat img)
{
	line(img, Point(0, Height/2), Point(Width, Height/2), Scalar(0,255,0), 1, CV_AA);
	line(img, Point(Width/2, 0), Point(Width/2, Height), Scalar(0,255,0), 1, CV_AA);
}

// 画比例尺
void CWJGLDlg::DrawImgScale(Mat img)
{
...
void CWJGLDlg::DrawLine(Mat img)
{
 ...
 void CWJGLDlg::DrawRect(Mat img)
 {
 ...
 void CWJGLDlg::DrawCircle(Mat img)
 {
...
void CWJGLDlg::MeasureDis(Mat img)
{
...
void CWJGLDlg::OnLButtonDown(UINT nFlags, CPoint point)
{
	// TODO: Add your message handler code here and/or call default
	CvPoint pt;
	pt.x = point.x - 10;
...
void CWJGLDlg::OnLButtonUp(UINT nFlags, CPoint point)
{
	// TODO: Add your message handler code here and/or call default
...
void CWJGLDlg::OnRButtonDown(UINT nFlags, CPoint point)
{
	// TODO: Add your message handler code here and/or call default
...
void CWJGLDlg::OnClickedSetStageXHomeBtn()
{
	// TODO: Add your control notification handler code here
...
void CWJGLDlg::OnClickedSetStageYHomeBtn()
{
	// TODO: Add your control notification handler code here
...
void CWJGLDlg::InitDMC3000Status()
{
...
void CWJGLDlg::SetDMC3000Status(BOOL status, int nID)
{
...
void CWJGLDlg::OnTimer(UINT_PTR nIDEvent)
{
	switch(nIDEvent)
	{
	case 0:
...
// 更新DMC3000运动状态
void CWJGLDlg::UpdateDMC3000Data()
{
...
// 更新距离
void CWJGLDlg::UpdateDistance(int nAxisIndex, int nDisID)
{
	UpdateData(TRUE);
	long currentPulse = dmc_get_position(g_nCardNo, nAxisIndex); //获取当前轴位置
...
void CWJGLDlg::OnClickedStageXStopBtn()
{
	// TODO: Add your control notification handler code here
...
void CWJGLDlg::OnClickedStageYStopBtn()
{
	// TODO: Add your control notification handler code here
